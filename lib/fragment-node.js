...
export class FragmentNode extends RootNode {
  ...
  async _render(_children, renderContext) {
    ...
    if (this.isIterableChild(_children))
      children = _children;
    else if (isJibish(_children) || this.isValidChild(_children))
      children = [ _children ];
    ...
    await Utils.iterateAsync(children, async ({ value: _child, key, index }) => {
      ...
      if (isJibish(child)) {
        ...
        return await fragmentNode.render(children, renderContext);
