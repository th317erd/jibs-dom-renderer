...
export class ComponentNode extends RootNode {
  ...
  onContextUpdate() {
    ...
    return Promise.resolve().then(async () => {
      ...
      await this.updateParent(this, renderResult, renderStartTime);
    });
  }
  ...
  async _render(jib, renderContext) {
    ...
    const finalizeRender = async (renderResult, didRender) => {
      ...
      let fragmentResult = await rootNode.render(renderResult, renderContext);
    ...
    try {
      if (!this.shouldRender(jib.props, jib.children)) {
        ...
        if (!component) {
          ...
          component.on(UPDATE_EVENT, async () => {
            let renderStartTime = Date.now();
            let renderResult    = await this.render(this._currentJib, this._cachedRenderContext || { index: 0 });
        ...
        if (Utils.instanceOf(renderResult, 'promise')) {
          ...
          renderResult.then(async (renderResult) => {
            ...
            let finalizedRenderResult = await finalizeRender(renderResult, true);
            this.updateParent(this, finalizedRenderResult, renderStartTime);
          }).catch(handleRenderError);

          return finalizeRender(waitingRenderResult || this._cachedRenderResult, true);
        } else {
