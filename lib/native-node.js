...
export class NativeNode extends RootNode {
  ...
  async destroy() {
    ...
    if (nativeElement) {
      if (await this.renderer.destroyNativeElement(nativeElement)) {
  ...
  async _render(jib, renderContext) {
    ...
    if (!nativeElement) {
      nativeElement = this.nativeElement = await this.renderer.createNativeElement(Type);
      await this.renderer.updateElementAttributes(nativeElement, props);
      ...
      let propDiff = await this.renderer.updateElementAttributes(nativeElement, props);
